...
export default defineMcpTool({
  ...
  description:
    'Remove one or more tags from a Bitrix24 task, keeping every other tag it has. Use it to retire a release marker ("R260916") or drop a priority tag without retyping the rest: `TAGS` is a whole-set field in Bitrix24, so removing one tag through `b24_task_update` means listing all the survivors and losing whatever you forget. Names are matched case-insensitively; names the task does not carry are reported in `notPresent` and change nothing. Read tags with `b24_task_list` (`select: ["tags"]`), add them with `b24_task_tag_add`. Bitrix24 indexes the tag filter with a short lag: a task tagged a second ago may not show up in `filter: {tag: …}` for a few seconds, so verify a fresh write by reading the task rather than by searching for it.',
  inputSchema: {
