...
const tool = (await import('../../../../server/mcp/tools/tasks/unwatch-task')).default as unknown as {
  handler: (input: { taskId: number }) => Promise<ToolContent>
}

describe('b24_task_unwatch', () => {
  beforeEach(() => {
    fake.v2Call.mockReset()
  })

  it('calls actions.v2.call.make with tasks.task.stopwatch and returns the unwatched-task summary', async () => {
    fake.v2Call.mockResolvedValue(fakeOk({ task: { id: 4573, title: 'stop watching me', status: '2', responsibleId: '9' } }))

    const result = await tool.handler({ taskId: 4573 })

    expect(fake.v2Call).toHaveBeenCalledWith({ method: 'tasks.task.stopwatch', params: { taskId: 4573 } })
